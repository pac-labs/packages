...
Outputs:
  dist/pac-full.zip
  dist/pac-patch.zip
  dist/pac-binaries.zip
  dist/RELEASE_BINARIES.json
  dist/PAC_RELEASE_MANIFEST.json
  dist/PAC_UPDATE_DIFF.diff when a previous git ref exists

GitHub is the PAC controller version authority. The workflow writes the generated
controller version into VERSION/PAC_CHANGELOG inside the checked-out workspace
before this script runs, but component binary versions remain owned by each
component and release artifact filenames stay stable to avoid local/manual
version skew. Compiled binaries are published as separate GitHub release assets;
PAC source/update zips intentionally do not bundle them.
...
def release_binary_files() -> tuple[list[tuple[Path, str]], list[dict[str, str]]]:
    """Return precompiled release binaries generated by compile-release-binaries.py."""
    out_root = DIST / "release-binaries"
    manifest_path = out_root / "RELEASE_BINARIES.json"
    if not manifest_path.exists():
        raise SystemExit(
            "Missing dist/release-binaries/RELEASE_BINARIES.json. "
            "Run scripts/compile-release-binaries.py before scripts/generate-pac-release.py."
        )
    try:
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise SystemExit(f"Invalid release binary manifest: {exc}") from exc
    binaries = manifest.get("binaries") if isinstance(manifest, dict) else []
    if not isinstance(binaries, list) or not binaries:
        raise SystemExit("Release binary manifest contains no binaries")
    extra_files: list[tuple[Path, str]] = []
    for path in sorted(out_root.rglob("*")):
        if path.is_file():
            extra_files.append((path, path.relative_to(DIST).as_posix()))
    return extra_files, [item for item in binaries if isinstance(item, dict)]
...
def copy_direct_binary_assets(release_binaries: list[dict[str, str]]) -> list[str]:
    """Copy each compiled binary to dist/<asset_name> for direct release upload."""
    copied: list[str] = []
    for record in release_binaries:
        asset_name = str(record.get("asset_name") or record.get("download_name") or "").strip()
        rel_path = str(record.get("path") or "").strip()
        if not asset_name or not rel_path:
            continue
        src = DIST / rel_path.removeprefix("dist/")
        if not src.exists():
            raise SystemExit(f"Release binary asset source missing: {src}")
        target = DIST / asset_name
        shutil.copy2(src, target)
        copied.append(asset_name)
    if not copied:
        raise SystemExit("No direct release binary assets were copied")
    return sorted(copied)


def write_binaries_zip(extra_files: list[tuple[Path, str]]) -> Path:
    out = DIST / "pac-binaries.zip"
    with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        for src_path, arcname in extra_files:
            zf.write(src_path, arcname)
    with zipfile.ZipFile(out) as zf:
        names = set(zf.namelist())
        if "release-binaries/RELEASE_BINARIES.json" not in names:
            raise SystemExit(f"{out.name} missing release binary manifest")
        if not any(name.startswith("release-binaries/") and name != "release-binaries/RELEASE_BINARIES.json" for name in names):
            raise SystemExit(f"{out.name} contains no binaries")
    manifest_src = DIST / "release-binaries" / "RELEASE_BINARIES.json"
    manifest_out = DIST / "RELEASE_BINARIES.json"
...
def main() -> int:
    ...
    extra_files, release_binaries = release_binary_files()
    full = DIST / "pac-full.zip"
    patch = DIST / "pac-patch.zip"
    write_zip(full)
    write_zip(patch)
    direct_binary_assets = copy_direct_binary_assets(release_binaries)
    binaries_zip = write_binaries_zip(extra_files)
    ...
    manifest = {
        ...
        "artifacts": {
            ...
            "binaries": binaries_zip.name,
            "binaries_manifest": "RELEASE_BINARIES.json",
            "direct_binary_assets": direct_binary_assets,
            "update_diff": update_diff.name,
        },
        "binary_delivery": {"mode": "direct-github-release-assets", "bundled_in_source_zips": False, "primary_binaries": ["pac-endpoint", "pacctl"]},
        "release_binaries": release_binaries,
        "changed_files": changed,
        "changelog_entry": next((e for e in changelog.get("releases", []) if isinstance(e, dict) and str(e.get("version")) == ver), None),
    }
